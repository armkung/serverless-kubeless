...
function getData(data, options) {
  ...
  try {
    if (!_.isEmpty(data)) {
      ...
      result = {
        body: JSON.parse(fs.readFileSync(opts.path, 'utf-8')),
        json: true,
      };
    }
  } catch (e) {
...
function invoke(func, data, funcsDesc, options) {
  ...
  const connectionOptions = helpers.getConnectionOptions(helpers.loadKubeConfig(), undefined, namespace);
    ...
    core.ns.services.get((err, servicesInfo) => {
      if (err) {
        ...
        if (_.isEmpty(functionService)) {
          console.log(`Not found any information about the function "${func}"`);
        }
